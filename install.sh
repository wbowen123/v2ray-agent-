...
readLocalTLS() {
    ...
    localTLSFullchainFile=
    localTLSKeyFile=
    scriptCurrentDir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P 2>/dev/null)
    ...
    certCandidates=(
        ...
        "${scriptCurrentDir}/certificate.pem"
    )
    keyCandidates=(
        ...
        "${scriptCurrentDir}/privatekey.pem"
    )
...
checkLocalTLSDomain() {
    local localDomain=$1
    local certSubjectAltName=
    local certSubject=

    if [[ -z "${localTLSFullchainFile}" || -z "${localDomain}" ]]; then
        return 1
    fi

    certSubjectAltName=$(openssl x509 -in "${localTLSFullchainFile}" -noout -text 2>/dev/null | grep -A 1 "Subject Alternative Name")
    certSubject=$(openssl x509 -in "${localTLSFullchainFile}" -noout -subject 2>/dev/null)

    if echo "${certSubjectAltName}" | grep -Fq "DNS:${localDomain}" || echo "${certSubject}" | grep -Fq "CN = ${localDomain}" || echo "${certSubject}" | grep -Fq "CN=${localDomain}"; then
        return 0
    fi
...
installLocalTLS() {
    ...
    echoContent green " ---> 检测到本地证书"
    echoContent yellow " ---> 证书文件: ${localTLSFullchainFile}"
    echoContent yellow " ---> 私钥文件: ${localTLSKeyFile}"

    if ! openssl x509 -in "${localTLSFullchainFile}" -noout >/dev/null 2>&1; then
        echoContent red " ---> 本地证书格式校验失败，跳过导入"
        return 1
    fi

    if ! openssl pkey -in "${localTLSKeyFile}" -noout >/dev/null 2>&1; then
        echoContent red " ---> 本地私钥格式校验失败，跳过导入"
        return 1
    fi

    certPublicKey=$(openssl x509 -in "${localTLSFullchainFile}" -pubkey -noout 2>/dev/null | openssl pkey -pubin -outform der 2>/dev/null | openssl dgst -sha256 2>/dev/null | awk '{print $NF}')
    keyPublicKey=$(openssl pkey -in "${localTLSKeyFile}" -pubout -outform der 2>/dev/null | openssl dgst -sha256 2>/dev/null | awk '{print $NF}')

    if [[ -z "${certPublicKey}" || -z "${keyPublicKey}" || "${certPublicKey}" != "${keyPublicKey}" ]]; then
        echoContent red " ---> 本地证书与私钥不匹配，跳过导入"
        return 1
    fi
    ...
    if [[ "${useLocalTLSStatus}" != "y" ]]; then
        echoContent yellow " ---> 已跳过本地证书，继续执行自动申请"
        return 1
    fi

    mkdir -p /etc/v2ray-agent/tls
    cp "${localTLSFullchainFile}" "/etc/v2ray-agent/tls/${tlsDomain}.crt"
    cp "${localTLSKeyFile}" "/etc/v2ray-agent/tls/${tlsDomain}.key"
    chmod 600 "/etc/v2ray-agent/tls/${tlsDomain}.crt" "/etc/v2ray-agent/tls/${tlsDomain}.key"

    if ! openssl x509 -in "/etc/v2ray-agent/tls/${tlsDomain}.crt" -noout >/dev/null 2>&1 || ! openssl pkey -in "/etc/v2ray-agent/tls/${tlsDomain}.key" -noout >/dev/null 2>&1; then
        rm -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" "/etc/v2ray-agent/tls/${tlsDomain}.key"
        echoContent red " ---> 本地证书导入失败，已回退"
        return 1
    fi
...
installTLS() {
    ...
    if [[ ! -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" || ! -f "/etc/v2ray-agent/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt" 2>/dev/null) || -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.key" 2>/dev/null) ]]; then
        if installLocalTLS "${tlsDomain}"; then
            return 0
        fi
    fi
